...
class Chart{
	...
	public function failedMail($data){
		...
		$subject .= $this->encode("��OOC���������ע��");
		
		$body = $this->encode("Chart��");
		$body .= $this->parameters['name']."\n";
		$body .= $this->encode("����ʱ�䣺".date('Y/m/d H:i:s')."\n");
		$against = $data['against'];
		if($against == 9) $against = 1;
		$body .= $this->encode("Υ������Rule".$against."\n");
		$body .= $this->encode("ԭʼ���ݣ�\n");		
	...
	private function rule1($args){
		if($this->parameters['type']<TYPE_P){//args������xbar��sub chart��ֵ
	...
	//rule 5:3��������2���㣬���������ߣ�ͬ�ࣩ����2����׼��
	private function rule5($args){		
		$v = $args['xbar'];	
		$ucl = $this->parameters['ucl_x'];
		$lcl = $this->parameters['lcl_x'];
		$cl = ($ucl+$lcl)/2;	
		$sigma2Up = ($cl+$ucl*2)/3;//+2����׼��
		$sigma2Low = ($cl+$lcl*2)/3;//-2����׼��
		$fieldInDB = 'xbar';
		if($this->parameters['type']==TYPE_IMR)
				$fieldInDB = 'x_1';
	...
	//rule 6:5��������4���㣬���������ߣ�ͬ�ࣩ����1����׼��
	private function rule6($args){		
		$v = $args['xbar'];	
		$ucl = $this->parameters['ucl_x'];
		$lcl = $this->parameters['lcl_x'];
		$cl = ($ucl+$lcl)/2;	
		$sigma1Up = ($cl*2+$ucl)/3;//+1����׼��
		$sigma1Low = ($cl*2+$lcl)/3;//-1����׼��
		$fieldInDB = 'xbar';
		if($this->parameters['type']==TYPE_IMR)
				$fieldInDB = 'x_1';
...
class Config_xml{
  ...
  public function __construct($id,$exist=false){
	...
	elseif ( file_exists(dirname(ABSPATH) . '/charts.xml') )
		...
		tx_die($this->encode("�Ҳ���<code>charts.xml</code>�ļ���������ɾ�����ļ��������Դ�<code>charts-sample.xml</code>���ơ�"));
  ...
  public function updateChart($args){
	...
	if($root->getAttribute('name') != $args['name']){
		$log .= "Chart������[".$this->decode($root->getAttribute('name'))."]��Ϊ[".$this->decode($args['name'])."];<br>";
		$root->setAttribute('name',$args['name']);		
	}
	if($root->getAttribute('team') != $args['team']){
		$log .= "����Team��[".$this->decode($root->getAttribute('team_name'))."]��Ϊ[".$this->decode($args['team_name'])."];<br>";
		...
		if($nodeName == 'description' || $nodeName == 'fileName' || $nodeName == 'linkName'){			
			...
			if($nodeName != 'logs'){
				if($childNode->nodeValue != $args[$nodeName]){
					...
					$childNode->nodeValue = $args[$nodeName];
					if($nodeName == 'rules')
						$log .= $nodeName."��[".substr($temp,2)."]��Ϊ[".substr($args[$nodeName],2)."];<br>";
					else
						$log .= $nodeName."��[$temp]��Ϊ[".$args[$nodeName]."];<br>";
				}
			}
		}
	}
