...
<div class="path" style="width:200px"><a href="tasks.php">我的任务</a> > 已完成的任务</div>
<div class='hr'></div>
<?php
...
if($_GET['t']){
	...
	if($_COOKIE['login_isAdmin'] == 2){		
		$total = $wpdb->get_var("SELECT count(*) FROM tasks WHERE complete=1 AND (rEmail='$email' OR cEmail='$email' OR uEmail='$email')");
	}else{
		$total = $wpdb->get_var("SELECT count(*) FROM tasks WHERE complete=1");
	}
}
...
if($_COOKIE['login_isAdmin'] == 2){
	$tasks = $wpdb->get_results("SELECT * FROM tasks WHERE complete=1 AND (rEmail='$email' OR cEmail='$email' OR uEmail='$email') ORDER BY lastTime DESC Limit $start,$pageSize", ARRAY_A);
}else{
	$tasks = $wpdb->get_results("SELECT * FROM tasks WHERE complete=1 ORDER BY lastTime DESC Limit $start,$pageSize", ARRAY_A);
}
